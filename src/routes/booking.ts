...
// get all bookings
router.get('/booking', asyncWrapper(async (req: Request, res: Response) => {
...
router.get('/booking/guestName', asyncWrapper(async (req: Request, res: Response) => {
  const booking = await prisma.booking.findFirst({
    where: {
      guestName: {
        contains: req.query.guestName as string
      }
    }
  });
...
// create booking
router.post('/booking', asyncWrapper(async (req: Request, res: Response) => {
  ...
  const booking = await prisma.booking.create({ data: { bookerName, bookerEmail, checkInDate: checkIn, checkOutDate: checkOut, guestName, totalRooms, roomTypeId, userId }});
...
router.put('/booking/:id', asyncWrapper(async (req: Request, res: Response) => {
...
// delete booking
router.delete('/booking/:id', asyncWrapper(async (req: Request, res: Response) => {
