...
// update user
router.put('/user/:id', mustLogin, upload.single('photo'), asyncWrapper(async (req: Request, res: Response) => {
...
// delete user
router.delete('/user/:id', mustLogin, asyncWrapper(async (req: Request, res: Response) => {
  const user = await prisma.user.findUnique({ where: { id: req.params.id }});
